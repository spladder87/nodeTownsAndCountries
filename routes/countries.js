  ...
  fs.readFile('stad.json', (err, data) => {
    ...
    for (var i = 0; i < towns.length; i++) {
      if (towns[i].countryid == req.params.id) {
        ...
        html += `</br><a href='/countries/towns/${towns[i].id}/delete'>Ta bort stad</a></br>`;
        html += "<hr>";
        html += "</li>";
      }
    }
    html += "</ul>";
    html += `<a href='/countries/${req.params.id}/towns/new'>Lägg till nytt stad</a></br>`;
    html += "<a href='javascript:history.back()'>Go Back</a>";
    res.send(html);
  })
...
//Delete town
router.get('/towns/:id/delete', urlencodedParser, function (req, res, next) {
